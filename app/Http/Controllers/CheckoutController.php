...
class CheckoutController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'sozlesme.accepted' => 'You must accept the Distance Sales Agreement and the Pre-Information Form.',
        ]);
            ...
            $order = DB::transaction(function () use ($data) {
                ...
                foreach (Cart::items() as $it) {
                    ...
                    if (! $product || $product->stock < $it['qty']) {
                        throw new RuntimeException('"' . $it['name'] . '" is out of stock. Please update your cart.');
                    }
    ...
    protected function sendOrderMails(Order $order): void
    {
        try {
            ...
            Log::error('Order email could not be sent', ['order' => $order->order_no, 'err' => $e->getMessage()]);
        }
    }
    ...
    public function callback(Request $request, Order $order)
    {
        ...
        if ($result) {
            ...
            Cart::clear();
            return redirect()->route('checkout.success', $order->order_no)->with('success', 'Your payment was received successfully.');
        }

        return redirect()->route('checkout.success', $order->order_no)->with('error', 'Payment could not be completed. Please try again or choose another method.');
    }
