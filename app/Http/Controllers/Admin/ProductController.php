...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data['ceviri'] = $this->buildCeviri($request, null);
    ...
    public function update(Request $request, Product $product)
    {
        ...
        $data['images'] = $data['cover'] ? [$data['cover']] : null;
        $data['attributes'] = $this->parseAttributes($request->input('attributes_raw'));
        $data['ceviri'] = $this->buildCeviri($request, $product);
    ...
    /**
     * EN çeviri alanlarını ceviri['en'] altına yazar; mevcut çevirileri (ör. attributes) korur.
     */
    protected function buildCeviri(Request $request, ?Product $product): ?array
    {
        $ceviri = $product?->ceviri ?? [];
        $en = $ceviri['en'] ?? [];

        foreach (['name', 'short_desc', 'description'] as $f) {
            $val = trim((string) $request->input('en_'.$f));
            if ($val !== '') {
                $en[$f] = $val;
            } else {
                unset($en[$f]);
