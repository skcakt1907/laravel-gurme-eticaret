...
class CategoryController extends Controller
{
    ...
    public function update(Request $request, Category $category)
    {
        ...
        $data['durum'] = $request->boolean('durum');
        $data['ceviri'] = $this->buildCeviri($request, $category);
    ...
    protected function buildCeviri(Request $request, ?Category $category): ?array
    {
        $ceviri = $category?->ceviri ?? [];
        $en = $ceviri['en'] ?? [];
        $val = trim((string) $request->input('en_name'));
        if ($val !== '') {
            $en['name'] = $val;
        } else {
            unset($en['name']);
