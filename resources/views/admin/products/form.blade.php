...
<form action="{{ $product->exists ? route('admin.products.update', $product) : route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="form-a">
    ...
    <div class="row g-4">
        <div class="col-lg-8">
            ...
            <div class="card-a mt-3" style="border-top:3px solid var(--ap)">
                <p style="font-weight:700;margin:0 0 .4rem"><i class="bi bi-translate"></i> English (EN) — İngilizce içerik</p>
                <p style="font-size:.82rem;color:#888;margin:0 0 .8rem">Boş bırakılan alanlar İngilizce sitede Türkçe değeriyle gösterilir.</p>

                <label>Product Name (EN)</label>
                <input name="en_name" value="{{ old('en_name', data_get($product->ceviri, 'en.name')) }}">

                <label>Short Description (EN)</label>
                <input name="en_short_desc" value="{{ old('en_short_desc', data_get($product->ceviri, 'en.short_desc')) }}">

                <label>Full Description (EN)</label>
                <textarea name="en_description" rows="6">{{ old('en_description', data_get($product->ceviri, 'en.description')) }}</textarea>
            </div>
        </div>
